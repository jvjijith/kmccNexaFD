...
import { usePostData } from '../../common/api'; // Import the custom hook for API calls

function CatalogCard({ children, title, button }) {
  const navigate = useNavigate();
  const { toggleSidebar } = useSidebar();

  // Use the mutation hook for adding catalog
  const { mutate: addCatalog, isPending: isAdding, error: addError } = usePostData("addCatalog", "/catalogues/add");

  // Function to handle adding the catalog
  const handleAddCatalog = () => {
    const catalogPayload = {
      "name": "New Catalog",
      "description": "Catalog Description",
      "catType": "manual",
      "productIds": [], // Replace with real product IDs
      "customParams": []
    };

    // Call the API to add the catalog
  ...
  return (
    <div className="flex w-full">
      ...
      <div className="h-screen flex-grow overflow-x-hidden overflow-auto flex flex-wrap content-start p-2">
        ...
        {addError && <p className="text-red-500">Error adding catalog: {addError.message}</p>}
      </div>
    </div>
  );
}
