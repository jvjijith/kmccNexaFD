...
function Header({ organization }) {
    ...
    return (
      <Document>
        <Page size="A4" style={styles.page}>          
          ...
          />
        <Text style={styles.mHeader}>Invoice</Text>
  ...
  <View style={{ flexDirection: "row", alignItems: "flex-start", justifyContent: "center" }}>
    ...
    {/* Quote Details */}
    <View style={{ flex: 1, marginRight: 10 }}>
      <Text style={{ textAlign: "left" }}>Invoice #: {quote.invoiceNumber}</Text>
      <Text style={{ textAlign: "left" }}>Created Date: {new Date(quote.createdAt).toLocaleDateString()}</Text>
      <Text style={{ textAlign: "left" }}>Valid Until: {new Date(quote.validUntil).toLocaleDateString()}</Text>
  ...
  <View style={{ flexDirection: "row", alignItems: "flex-start", justifyContent: "center" }}>
    ...
    <View style={{ flex: 1, marginRight: 10 }}>
      ...
      <Text style={{ textAlign: "left" }}>{customer.billingAddress}</Text>
      <Text style={{ textAlign: "left" }}>
        {customer.location}, {customer.country}
      </Text>
      <Text style={{ textAlign: "left" }}>Email: {customer.email}</Text>
      <Text style={{ textAlign: "left" }}>Phone: {customer.phone}</Text>
    </View>
    ...
    <View style={{ flex: 1, marginLeft: 10 }}>
      ...
      <Text style={{ textAlign: "left" }}>{customer.name}</Text>
      <Text style={{ textAlign: "left" }}>{customer.shippingAddress}</Text>
      <Text style={{ textAlign: "left" }}>
        {customer.location}, {customer.country}
      </Text>
      <Text style={{ textAlign: "left" }}>Email: {customer.email}</Text>
...
function SalesInvoiceTable() {
  ...
  const { data: salesInvoiceData, isLoading, error, refetch } = useGetData(
    "SalesInvoiceData",
    `/salesInvoices?page=${currentPage}&limit=${limit}`,
    {}
  );
